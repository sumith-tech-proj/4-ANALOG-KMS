...
public class clsSystemInfo {
    ...
    public static void getSystemInfo() {
        try {
            clsSharedVariables.cpu_part_no = SystemInfo.getModelName();// "CORTEX-A53 (ARM v8)";
            clsSharedVariables.cpu_qualification = SystemInfo.getHardware();

            try {
                clsSharedVariables.os_version = SystemInfo.getOsVersion();
            } catch (UnsupportedOperationException ex) {
            }

            String hardware_info = SystemInfo.getHardware().trim();
            try {
                if (hardware_info.startsWith(clsDefines.TINKER_BOARD_HARDWARE)) {
                    clsDefines.OBU_BRD_TYPE = clsDefines.TINKER_BOARD;
                } else if (hardware_info.startsWith(clsDefines.RASPBERRYPI_BOARD_HARDWARE)) {
                    clsDefines.OBU_BRD_TYPE = clsDefines.RASPBERRY_BOARD;
                } else if (hardware_info.startsWith(clsDefines.BANANAPI_BOARD_HARDWARE)) {

                    clsDefines.OBU_BRD_TYPE = clsDefines.BANANAPI_BOARD;
                }
            } catch (UnsupportedOperationException ex) {
            }
            /*
